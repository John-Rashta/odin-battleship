...
const pageLoad = function pageSetupAndManagement() {
  ...
  const playerBoard = function loadPlayerBoardIntoPage(player) {
    domManager.setupPlayerBoard(
      player.gameboard.getMissedHits(),
      player.gameboard.getShipHits(),
      player.gameboard.getShips(),
    );
  };
  const opponentBoard = function loadOpponentBoardIntoPage(player) {
    domManager.setupOpponentBoard(
      player.gameboard.getMissedHits(),
      player.gameboard.getShipHits(),
      player.gameboard.getShips(),
    );
      ...
      opponentGrid.addEventListener("click", (e) => {
        if (gameOver) {
          ...
          if (currentPlayer === computerPlayer) {
            ...
            do {
              if (hitRecord === "hit") {
                const newCoord = computerPlayer.getCloseCoord(
                  realPlayer.gameboard.getAllHits(),
                  lastCoord,
                );
                ...
                const newCoord = computerPlayer.getRandomCoord(
                  realPlayer.gameboard.getAllHits(),
                );
                const newRecord = realPlayer.gameboard.receiveAttack(newCoord);
                playerBoard(realPlayer);
              ...
              (hitRecord === "hit" && !gameOver) ||
              (hitRecord === "sunk" && !gameOver)
            );
            currentPlayer = realPlayer;
          }
        }
