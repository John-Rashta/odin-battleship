export default function SetupDom() {
  const helperCheckCells = function checkCellsBeforeDroppingShip(shipCoords) {
    ...
    currentShip.forEach((coord) => {
      const currentCell = document.querySelector(
        `.cell[data-y="${coord[0]}"][data-x="${coord[1]}"]`,
      );
  ...
  const helperGetShipCoords = function getShipsCoordsFromDom(
    headCoord,
    direction,
    size,
  ) {
  ...
  const helperMissedHits = function toggleMissedHitsCells(
    missedHits,
    currentField,
  ) {
    missedHits.forEach((hit) => {
      const currentCell = currentField.querySelector(
        `.cell[data-y="${hit[0]}"][data-x="${hit[1]}"]`,
      );
  ...
  const helperShips = function toggleShipsCells(ship, currentField) {
    ...
    const shipHead = currentField.querySelector(
      `.cell[data-y="${firstCoord[0]}"][data-x="${firstCoord[1]}"]`,
    );
  ...
  const allowDrag = function allowShipsToBeDragged(e) {
    ...
    const removeEvents = function removeAllEvents(e) {
      if (
        ...
        if (
          (oppositeDirection === "h" &&
            Number(parentDiv.dataset.x) + (Number(shipDiv.dataset.size) - 1) >
              9) ||
          (oppositeDirection === "v" &&
            Number(parentDiv.dataset.y) - (Number(shipDiv.dataset.size) - 1) <
              0)
        ) {
      ...
      const [currentDiv, divBellow] = document.elementsFromPoint(
        currentX,
        currentY,
      );
      ...
      if (divBellow.classList.contains("cell")) {
        if (
          (shipDiv.dataset.direction === "h" &&
            Number(divBellow.dataset.x) + (Number(shipDiv.dataset.size) - 1) >
              9) ||
          (shipDiv.dataset.direction === "v" &&
            Number(divBellow.dataset.y) - (Number(shipDiv.dataset.size) - 1) <
              0)
        ) {
      ...
      helperToggle(helperGetShipCoords([parentY, parentX], direction, size));
      shipDiv.style.zIndex = "2";
      bodyDiv.removeEventListener("mousemove", dragElement);
      bodyDiv.removeEventListener("mouseup", removeEvents);
    };
  ...
  const changeButton = function changeStartButtonToRestart() {
    ...
    startButton.textContent = startButton.classList.contains("startButton")
      ? "Restart"
      : "Start";
  ...
  const createStartScreen = function createGridToPlaceShips() {
    ...
    secondInfo.textContent =
      "Click the ships to swap directions between horizontal and vertical.";
    ...
    defaultShips.forEach((ship) => {
      ...
      const shipHead = document.querySelector(
        `.cell[data-y="${firstCoord[0]}"][data-x="${firstCoord[1]}"]`,
      );
