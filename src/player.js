...
export default function setupPlayer() {
  ...
  const getRandomShip = function getARandomShipFromCoordWithSpecifiedLength(
    head,
    length,
    direction,
  ) {
  ...
  const getCloseCoord = function getAdjacentCoord(avoidCoords, lastCoord) {
    ...
    for (let i = 0; i < 8; i++) {
      if (
        yArray[i] + firstCoord[0] > 9 ||
        yArray[i] + firstCoord[0] < 0 ||
        xArray[i] + firstCoord[1] > 9 ||
        xArray[i] + firstCoord[1] < 0
      ) {
        continue;
      }
